...
function SubscriptionRow({ item, onEdit, onToggle, onRemove }: { item: Subscription; onEdit: (item: Subscription) => void; onToggle: (item: Subscription) => Promise<void>; onRemove: (id: string) => Promise<void> }) {
  ...
  return <Card className={`min-w-0 border-0 bg-surface shadow-vault ${active ? '' : 'opacity-65'}`}><div className="grid min-w-0 grid-cols-[auto_minmax(0,1fr)] items-center gap-3 p-4 sm:grid-cols-[auto_minmax(0,1fr)_auto_auto]"><div className={`flex size-10 shrink-0 items-center justify-center rounded-xl ${active ? 'bg-primary/15 text-primary' : 'bg-surface-container-high text-muted-gray'}`}>{active ? <CheckCircle2 className="size-5" /> : <XCircle className="size-5" />}</div><div className="min-w-0"><p className="truncate font-medium text-on-surface">{item.name}</p><p className="mt-0.5 flex items-center gap-1 text-xs text-muted-gray"><CalendarDays className="size-3" /> Cada mes, día {item.billingDay}</p></div><p className="col-span-2 font-semibold text-on-surface sm:col-span-1">{formatMoney(item.amount)}<span className="text-xs font-normal text-muted-gray">/mes</span></p><div className="col-span-2 flex justify-end gap-1 border-t border-graphite/70 pt-2 sm:col-span-1 sm:border-0 sm:pt-0"><Button variant="ghost" size="icon" onClick={() => onEdit(item)} title="Editar"><Pencil className="size-4" /></Button><Button variant="ghost" size="icon" onClick={() => void onToggle(item)} title={active ? 'Cancelar suscripción' : 'Reactivar suscripción'}>{active ? <XCircle className="size-4 text-warning" /> : <RotateCcw className="size-4 text-primary" />}</Button><Button variant="ghost" size="icon" onClick={() => void onRemove(item.id)} title="Eliminar"><Trash2 className="size-4 text-error" /></Button></div></div></Card>
}
