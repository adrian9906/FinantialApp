...
const current = { currencies: [usd, cup], activeCurrencyCode: 'CUP', activeIncomeSourceId: 'account1', accountSavingsFormulas: { account1: formula }, formula }
...
assert.equal(mergeCurrencyPreferences(current, { activeIncomeSourceId: 'account2' }).activeIncomeSourceId, 'account2')
assert.equal(mergeCurrencyPreferences(current, { currencies: [{ ...cup, exchangeRate: 400 }] }).activeIncomeSourceId, 'account1')
