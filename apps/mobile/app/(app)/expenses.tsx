...
import { ArrowUpRight, Dumbbell, HeartPulse, House, Package, Pencil, Plus, ShoppingBasket, Trash2 } from 'lucide-react-native'
...
export default function ExpensesScreen() {
  ...
  async function handleSave() {
    ...
    if (nextAmount > availableToPlan || plannedTotal + nextAmount > overview.budgetExpenses) {
      setFormError(`No puedes agregarlo porque supera el limite disponible de ${formatMoney(availableToPlan)}.`)
      return
    }

    const currentStatus = editId ? expenseItems.find((item) => item.id === editId)?.status ?? 'pending' : 'pending'
    const payload = {
      amount: nextAmount,
  ...
  return (
    <AppFrame
      title="Gastos"
      subtitle="Lista esencial por categorias, con presupuesto mensual y check real para descontar del overview."
      actions={
    ...
    >
        ...
        <CardContent className="pt-6">
          <View style={{ gap: spacing.md }}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
              <Text style={{ color: palette.textMuted, fontSize: 12, textTransform: 'uppercase' }}>Presupuesto mensual ({formula.expenses}%)</Text>
            ...
            <Text style={{ color: palette.text, fontSize: 28, fontWeight: '800' }}>
              {formatMoney(overview.totalExpenses)} <Text style={{ color: palette.textMuted, fontSize: 16 }}>/ {formatMoney(overview.budgetExpenses)}</Text>
            </Text>
            <View style={{ height: 10, borderRadius: radius.full, overflow: 'hidden', backgroundColor: palette.surfaceMuted }}>
              <View style={{ height: '100%', width: `${pct}%`, backgroundColor: palette.primary }} />
            </View>
            <View style={{ flexDirection: 'row', gap: spacing.md }}>
              <Text style={{ color: palette.textMuted, fontSize: 12 }}>Productos: {expenseCount}</Text>
        ...
        return (
          <Card key={key}>
            ...
            <CardContent className="gap-3">
              ...
              {items.length === 0 ? (
                <Text style={{ color: palette.textMuted, fontSize: 12 }}>Aun no hay productos en esta categoria.</Text>
              ) : items.map((item) => (
                <Pressable key={item.id} onPress={() => void toggleChecked(item)} style={{ borderRadius: radius.md, borderWidth: 1, borderColor: item.status === 'checked' ? meta.color : palette.border, backgroundColor: palette.backgroundAlt, padding: spacing.md }}>
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', gap: spacing.md }}>
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={{ color: palette.textMuted, fontSize: 12 }}>{item.date} · {item.status === 'checked' ? 'Comprado' : 'Pendiente'}</Text>
                    </View>
      ...
      <Dialog
        ...
        description="Guarda cada producto esencial como un gasto individual."
        footer={
