  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <header className="flex items-center justify-between">
        <div>
          <h1 className="text-[28px] md:text-[36px] font-semibold text-on-surface tracking-tight">Savings</h1>
          <p className="text-sm text-muted-gray">25% of your income — future you</p>
        </div>
        <Button onClick={() => setOpen(true)} className="bg-tertiary-container text-white hover:brightness-110 shadow-vault">
          <Plus className="size-4" /> Add Saving
        </Button>
      </header>
        ...
        <div className="relative z-10">
          <div className="flex justify-between items-center mb-2">
            ...
            <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${remaining >= 0 ? 'bg-success/10 text-success' : 'bg-tertiary-container/20 text-tertiary'}`}>
              {remaining >= 0 ? `$${remaining.toLocaleString()} left to save` : `$${Math.abs(remaining).toLocaleString()} over saved`}
            </span>
          </div>
      ...
      {savingsList.length === 0 ? (
        <Card className="bg-surface border-0 shadow-vault">
          <div className="flex flex-col items-center gap-3 py-16 text-muted-gray text-sm">
            ...
            <Button variant="secondary" onClick={() => setOpen(true)} className="bg-surface-container-high text-on-surface">Record a saving</Button>
          </div>
        </Card>
      ) : (
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="bg-surface border-graphite">
          <DialogHeader>
            <DialogTitle className="text-on-surface">Add Saving</DialogTitle>
            <DialogDescription>Track money saved this month</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input placeholder="Emergency fund" value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} className="bg-abyss border-graphite text-on-surface" />
            </div>
            <div className="space-y-2">
